...
export class BookingService {
  ...
  async cancelStaleUnpaidBookings() {
    ...
    for (const booking of bookings) {
      ...
      // Notify both guest and owner about cancellation
      if (booking.guest?.email) {
        await this.notification.sendBookingRejected(
          booking.guest.email,
          booking.id,
        );
      }
      if (booking.villa?.owner?.email) {
        await this.notification.sendBookingRejected(
          booking.villa.owner.email,
          booking.id,
        );
      }
      ...
      if (booking.villa?.owner?.phone) {
        await this.notification.sendBookingRejectedWhatsApp(
          booking.villa.owner.phone,
          booking.id,
        );
      }
  ...
  async createBooking(userId: string, dto: CreateBookingDto) {
    ...
    await this.notification.sendBookingCreated(
      booking.villa.owner.email,
      booking.id,
    );
    ...
    // Send WhatsApp notifications
    if (booking.guest?.phone) {
      await this.notification.sendBookingCreatedWhatsApp(
        booking.guest.phone,
        booking.id,
      );
    }
  ...
  async confirmBooking(bookingId: string, ownerId: string) {
    ...
    if (guest?.email) {
      await this.notification.sendBookingInvoicePdf(guest.email, fullBooking);
    }
